...
test('all cards, readable beats and five-round victory',async({page})=>{
 ...
 await page.getByRole('button',{name:'CONTINUE WORKDAY'}).click();await page.clock.runFor(500);await expect(page.getByRole('heading',{name:'YOUR WORKDAY'})).toBeVisible();await expect(page.locator('.workday-screen')).toHaveCSS('opacity','1');await expect(page.locator('.run-clock')).toContainText('09:47');await expect(page.locator('.schedule-done').filter({hasText:'THE SCRUM LORD'})).toBeVisible();await expect(page.locator('.employee-condition')).toContainText('MENTAL CAPACITY');await expect(page.locator('.employee-condition')).toContainText(`${remaining} / 100`);await expect(page.getByText('WORKDAY PROTOTYPE ENDS HERE')).toBeVisible();await page.screenshot({path:'artifacts/phase4b-post-victory.png',fullPage:true,animations:'disabled'});
...
test('real-time full battles, rematch reset and back to title',async({page})=>{
 ...
 await page.getByRole('button',{name:'CONTINUE WORKDAY'}).click();await expect(page.getByText('WORKDAY PROTOTYPE ENDS HERE')).toBeVisible();await page.getByRole('button',{name:'NEW EMPLOYEE'}).click();await expect(page.getByRole('heading',{name:'WHO ARE YOU CLOCKING IN AS?'})).toBeVisible();
});
