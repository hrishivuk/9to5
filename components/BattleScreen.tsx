...
import {WORKDAY_STORAGE_KEY,completeScrumLord,createWorkdayRun,failWorkday,loadWorkdayRun,type WorkdayRun} from '@/lib/workday';
const TUTORIAL_KEY='9to5-battle-tutorial-seen';
const STATUS_TUTORIAL_KEY='9to5-status-tutorial-seen';
export default function BattleScreen(){
 const [screen,setScreen]=useState<'title'|'create'|'reveal'|'workday'|'calendar'|'vs'|'battle'>('title');
 ...
 useEffect(()=>{const saved=loadWorkdayRun(localStorage.getItem(WORKDAY_STORAGE_KEY));if(!saved)return;setWorkday(saved);setFighter(saved.fighter);setState(initialBattle(saved.mentalCapacity));setScreen('workday');},[]);
 ...
 function resetBattle(_next:FighterProfile,health=workday?.mentalCapacity??100){
  sequence.current++;
  setState(initialBattle(health));setLogs([{id:0,round:0,text:'You joined the meeting. He chose violence.',detail:'Select a card to begin.'}]);
 ...
 function continueWorkday(){if(!workday)return;const next=state.result==='win'?completeScrumLord(workday,state.player):failWorkday(workday);saveRun(next);setShowResult(false);setScreen('workday');}
