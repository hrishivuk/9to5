...
function ActionReadout({beat,busy,headline}:{beat:Beat|null;busy:boolean;headline:string}){
 ...
 if(beat.kind==='player')return beat.enemyDamage?<><span className="arena-level">{beat.text}</span><strong>−{beat.enemyDamage}</strong>{beat.equipmentBonus?<small className="damage-math">BASE DAMAGE&nbsp; {beat.baseDamage}<br/>{beat.equipmentLabel}&nbsp; +{beat.equipmentBonus}<br/><b>TOTAL&nbsp; {beat.enemyDamage}</b><br/>SCRUM LORD&nbsp; {beat.beforeEnemy} → {beat.state.enemy}</small>:<small>SCRUM LORD&nbsp;&nbsp; {beat.beforeEnemy} → {beat.state.enemy}</small>}</>:<><span className="arena-level">STATUS ADDED</span><strong>CANNOT REPRODUCE</strong><small>NEXT ATTACK · 70% BLOCKED</small></>;
 ...
 if(beat.kind==='enemy')return <><span className="arena-level">{beat.text}</span><strong>−{beat.playerDamage}</strong>{beat.statusBonus||beat.blocked||beat.equipmentReduction?<small className="damage-math">{beat.statusBonus?<><span>BASE DAMAGE&nbsp; {beat.baseDamage}</span><br/>SCOPE CREEP&nbsp; +{beat.statusBonus}<br/><span>TOTAL&nbsp; {beat.incoming}</span></>:<>INCOMING&nbsp; {beat.incoming}</>}{beat.equipmentReduction?<><br/>{beat.equipmentLabel}&nbsp; −{beat.equipmentReduction}</>:null}{beat.blocked>beat.equipmentReduction?<><br/>BLOCKED&nbsp; −{beat.blocked-beat.equipmentReduction}</>:null}<br/><b>DAMAGE&nbsp; {beat.playerDamage}</b><br/>YOU&nbsp; {beat.beforePlayer} → {beat.state.player}</small>:<small>YOU&nbsp;&nbsp; {beat.beforePlayer} → {beat.state.player}</small>}</>;
